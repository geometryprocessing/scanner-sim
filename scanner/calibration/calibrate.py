...
def calibrate(obj_points, img_points, dim, error_thr=1.0, mtx_guess=None, no_tangent=True, out_dir="", plot=False, save_figures=True):
    ...
    print("Mean initial error:", np.mean(initial_errors))

    selected = np.nonzero(initial_errors < error_thr)[0]
    ...
    print("Mean selected error:", np.mean(refined_errors))
    ...
    if plot:
        ...
        plt.clf()
        plt.plot(np.arange(n), initial_errors, ".r", label="Initial")
        plt.plot(selected, refined_errors, ".b", label="Refined")
        plt.plot([0, n], [error_thr, error_thr], '--k', label="Threshold")
        plt.title("Projection Errors")
        plt.xlabel("Image #")
        plt.ylabel("Error, pixels")
        plt.ylim([0, 1.1 * np.max(initial_errors)])
        plt.legend()
        plt.tight_layout()
        if save_figures:
...
def reconstruct_planes(data_path, camera_calib, full_offsets=None, half_offsets=None):
    ...
    projection_errors = []
    for i in sorted([int(name[name.rfind("_") + 1:-4]) for name in charuco.keys()]):
        name, pair = charuco_template % i, checker_template % i
        ...
        projection = cv2.projectPoints(c_obj, rvec, tvec, cam_new_mtx, None)[0].reshape(-1, 2)
        projection_errors.extend(np.linalg.norm(c_img - projection, axis=1).tolist())
    ...
    return (charuco_3d, charuco_id, charuco_frame), (checker_3d, checker_2d, checker_local), projection_errors
