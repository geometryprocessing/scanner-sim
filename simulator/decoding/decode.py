...
from utils import load_openexr, linear_map, ensure_exists, plot_image, plot_hist
...
def get_single_bit_mask(filename, inverted_filename, undistort=None, plot=False, **kw):
    image, _ = load_openexr(filename, make_gray=True)
    inverted, _ = load_openexr(inverted_filename, make_gray=True)
...
#    jobs = [joblib.delayed(get_single_bit_mask, check_pickle=False)
 #           (filename, inverted_filename, undistort=undistort, plot=False, **kw)
    ...
    for filename, inverted_filename in zip(filenames, inverted_filenames):#]
        results.append(get_single_bit_mask(filename, inverted_filename, undistort=undistort, plot=False, *kw))

    return np.array(results)
    #return np.array(joblib.Parallel(verbose=15, n_jobs=-1, batch_size=1, pre_dispatch="all")(jobs))


def decode_gray(data_path, symmetric=True, out_dir="decoded", mask_sigma=3, mask_iter=6, crop=None, offset=-150,
                  undistort=None, group=False, save=True, plot=False, threshold=50, save_figures=True, verbose=False, sim=False, **kw):
    
    if sim:
        pattern = "/img_%03d.exr"
        white_p = "/img_000.exr"
        black_p = "/img_001.exr"
    else:
        pattern = "/img_%02d.exr"
        white_p = "/img_00.exr"
        black_p = "/img_01.exr"
        

    if symmetric:
        all_names = [pattern % i for i in range(46)]
    ...
    if symmetric:
        white, depth_gt = load_openexr(data_path + white_p, make_gray=True, load_depth=sim)
        blank, _ = load_openexr(data_path + black_p, make_gray=True)
    else:
    ...
    #thr_otsu, mask = cv2.threshold(ldr, 200, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    thr_otsu, mask = cv2.threshold(ldr, threshold, 255, cv2.THRESH_BINARY)
    ...
    if plot:
        ...
        img2 = img.copy()
        img2[r, c] = p_r
        plot_image(img2, "Horizontal", data_path + " - Decoded Horizontal", save_as=save_path + "horizontal" if save_path else None)
        img3 = img.copy()
        img3[r, c] = p_c
        plot_image(img3, "Vertical", data_path + " - Decoded Vertical", save_as=save_path + "vertical" if save_path else None)
