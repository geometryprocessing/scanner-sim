...
from utils import numpinize, load_openexr, save_openexr



def create_color_hdr(data_path, white_balance_path="scanner/calibration/projector/white_balance.json"):
    wb = numpinize(json.load(open(white_balance_path)))
    d, _ = load_openexr(data_path + "gray/img_01.exr")
    r, _ = load_openexr(data_path + "color/red.exr")
    r -= d
    g, _ = load_openexr(data_path + "color/green.exr")
    g -= d
    b, _ = load_openexr(data_path + "color/blue.exr")
    b -= d
    # g_r = np.average(g/r)
    # g_b = np.average(g/b)
    # print(g_r, g_b)
    ...
    print(rgb.shape)
    filename = data_path + "gray/img_color.exr"
