...
def configure_camera_geometry(config, cam_geom):
    ...
    config["pixelAspect"] = cam_geom["new_mtx"][0, 0] / cam_geom["new_mtx"][1, 1]

    # TODO: make use of cam_geom["new_mtx"][:2, 2] and cropping feature in mitsuba to simulate optical axis offset


def configure_camera_focus(config, cam_focus, tolerances_factor=1.4, max_wavelength_nn=650):
    ...
    config["diffLimit"] = 1.22 * max_wavelength_nn * 1.e-9 / (2. * config["cam_aperture"])
    # Account for manufacturing tolerances of the lens and convert from radians to degrees
    config["diffLimit"] *= tolerances_factor * (180. / np.pi)
    config["diffLimit"] = 2.65  # Temporary override (in-pixels value)


def configure_projector_geometry(config, proj_geom):
    pass


def configure_projector_focus(config, proj_focus):
    pass
